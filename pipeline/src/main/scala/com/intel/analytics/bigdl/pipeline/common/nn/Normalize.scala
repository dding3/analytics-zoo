 ...
 * todo: in caffe this scale is learnable!
 */
@SerialVersionUID(1504221556573977764L)
class Normalize[T: ClassTag](val p: Double, val eps: Double = 1e-10, scale: Double = 1)
  ...
  override def updateOutput(input: Tensor[T]): Tensor[T] = {
    ...
    if (norm.dim() <= 2) {
      ...
      output.cdiv(inputBuffer, norm.view(norm.size()).expandAs(inputBuffer).contiguous())
    }
    if (scale > 1) output.mul(ev.fromType(scale))
...
object Normalize {
  def apply[@specialized(Float, Double) T: ClassTag](
    p: Double, eps: Double = 1e-10, scale: Double = 1)
    (implicit ev: TensorNumeric[T]): Normalize[T] = {
    new Normalize(p, eps, scale)
  }
}
