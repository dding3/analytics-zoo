...
import com.intel.analytics.bigdl.pipeline.common.PascalVocEvaluator._
import com.intel.analytics.bigdl.pipeline.common.dataset.PascalVoc._
...
class PascalVocEvaluator(imageSet: String) extends DetectionEvaluator {
  ...
  def map(results: Array[(Int, Array[(Float, Int, Int)])]): Array[(String, Double)] = {
    ...
    while (i < classes.length) {
      ...
      if (cls != "__background__") {
        val ap = EvalUtil.map(results(i)._2, use07Metric = use07metric, results(i)._1)
        aps.append(ap)
  ...
  def evaluateBatch(results: Array[Array[Target]], gt: Tensor[Float])
